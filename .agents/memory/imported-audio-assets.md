...
The imported MP3 asset metadata can contain `/__l5e/assets-v1/...` URLs that are not served by this Replit project. Until the actual recordings are added to the workspace, lesson playback should use the European Portuguese browser speech-synthesis fallback rather than requesting those paths. Replay speech must be initiated directly from a user gesture because delayed timer callbacks can be treated as autoplay.
...
**How to apply:** Before wiring imported audio metadata into lesson data or the download ZIP, confirm the corresponding recording exists as a project-local asset and is served successfully in the Replit preview. Keep replay actions on the final user tap and resume speech/audio contexts during touch handling.